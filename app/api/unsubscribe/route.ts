...
export async function GET(request: NextRequest) {
  ...
  const res = await fetch(`${url}/rest/v1/waitlist_gbucht?confirm_token=eq.${token}&select=id`, {
    headers: { 'apikey': key, 'Authorization': `Bearer ${key}` },
  });
  const entries = await res.json();
  if (!entries.length) return new NextResponse(page('❌','Bereits abgemeldet.'), { status:404, headers:{'Content-Type':'text/html'} });

  await fetch(`${url}/rest/v1/waitlist_gbucht?confirm_token=eq.${token}`, {
    method:'DELETE', headers:{'apikey':key,'Authorization':`Bearer ${key}`},
  });
  return new NextResponse(page('👋','Abgemeldet. Schade — aber kein Drama.'), { status:200, headers:{'Content-Type':'text/html'} });
}
